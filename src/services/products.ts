...
const priceCache: Record<
  string,
  { price: number; timestamp: number; __isEstimate?: boolean }
> = {};
...
async function fetchRealPrice(
  ...
  if (
    priceCache[cacheKey] &&
    now - priceCache[cacheKey].timestamp < CACHE_EXPIRY
  ) {
  ...
  try {
    ...
    window.__apiFailureCount++;
    console.error(
      `Error fetching real price for ${itemName} at ${storeName}:`,
      error
    );
    const price = await estimatePrice(itemName, storeName);
    priceCache[cacheKey] = {
...
export const searchProducts = async (
  ...
  try {
    ...
    const displayName =
      storePatterns[itemName.toLowerCase()] ||
    ...
    try {
      ...
      if (realPrice !== null) {
        ...
        isEstimate =
          !priceCache[cacheKey] || priceCache[cacheKey].__isEstimate === true;

      ...
      .then(() => {
        try {
          ...
          if (window.google.maps.places?.Place) {
            ...
            console.warn(
              "Google Maps Place API not available, will use legacy API"
            );
          }
...
export const searchNearbyStores = async (
  ...
  try {
    ...
    // Cast to any to bypass strict type check, as Google Maps types may be out of sync
...
const searchNearbyStoresLegacy = async (
  ...
  const searchWithRadius = async (searchRadius: number): Promise<any[]> => {
    return new Promise((resolve, reject) => {
      ...
      try {
        tempService.nearbySearch(request, (results, status) => {

...
async function scrapeGoogleShopping(item, locationHint = '') {
  ...
  try {
    ...
    const searchBox = await page.waitForSelector('input[name="q"], textarea[name="q"]', { timeout: 10000 }).catch(e => {
    ...
    const products = await page.evaluate(({ searchedItem, storeFilter }) => {
      ...
      const extractDistance = (text) => {
        if (!text) return null;
        const match = text.match(/(\d+(\.\d+)?)\s*mi away/i) || 
      ...
      function findProductName(container, priceElement) {
        ...
        for (const selector of nameSelectors) {
          const element = container.querySelector(selector);
          if (element && element !== priceElement && !isUIElement(element.textContent)) {
            const name = extractTextContent(element);
            if (name && name.length > 3 && !name.includes("Nearby,") && !name.includes("Also nearby")) {
              return name;
            }
          }
        }
      ...
      function findStoreName(container, priceElement, nameElement) {
        ...
        const potentialStoreEls = Array.from(container.querySelectorAll('*')).filter(el => 
          el !== priceElement && 
          el !== nameElement && 
      ...
      function findDistance(container) {
        ...
        const nearbyMatch = container.textContent.match(/Nearby,\s+(\d+(\.\d+)?)\s*mi/i);
        if (nearbyMatch) {
        ...
        const distanceEls = Array.from(container.querySelectorAll('*')).filter(el => 
          /\d+(\.\d+)?\s*mi/.test(el.textContent)
        );
      ...
      for (const selector of productSelectors) {
        ...
        if (productCards.length > 0) {
          console.log(`Found ${productCards.length} potential products with selector ${selector}`);
          ...
          productCards.forEach((card) => {
            try {
              ...
              if (name && price) {
                ...
                if (storeFilter && store && !store.toLowerCase().includes(storeFilter.toLowerCase())) {
      ...
      if (results.length === 0) {
        console.log("No products found with standard selectors, trying fallback method");
        ...
        console.log(`Found ${potentialContainers.length} potential product containers`);
      ...
      if (results.length === 0) {
        console.log("Still no products found, trying final fallback with just price elements");
      ...
      results.forEach(product => {
        ...
        if (product.store) {
          ...
          if (product.store.includes("returns") || 
